...
import alpaca_trade_api as tradeapi  # Alpaca for paper trading
import webbrowser
...
def save_dataframe_to_csv(df, filename):
    ...
    try:
        ...
        logging.error(f"Permission denied when saving file: {filename}")
    except Exception as e:
        logging.error(f"Error saving DataFrame to {filename}: {e}")

# Alpaca API setup (Replace with your API keys)
ALPACA_API_KEY = "REDACTED_ALPACA_KEY_4"
ALPACA_SECRET_KEY = "REDACTED_ALPACA_SECRET_4"
...
class TwitterSentimentAnalysis:
    ...
    def fetch_sentiment(ticker):
        """
        Fetch sentiment for a given ticker using dummy sentiment analysis (TextBlob or placeholder).
        """
        try:
            # Placeholder sentiment analysis (you can improve with a real API)
...
class CoinGeckoAPI:
    ...
    def fetch_latest_price(ticker):
        ...
        try:
            ...
            response = requests.get(url, params=params).json()
            if ticker in response and "usd" in response[ticker]:
                return float(response[ticker]["usd"])
            else:
...
def execute_trade(ticker, quantity, side="buy"):
    ...
    try:
        price = (
            alpaca.get_last_trade(ticker).price
            if side == "buy"
            else alpaca.get_last_trade(ticker).price
        )
        if side == "buy":
            alpaca.submit_order(symbol=ticker, qty=quantity, side="buy", type="market", time_in_force="gtc")
        elif side == "sell":
            alpaca.submit_order(symbol=ticker, qty=quantity, side="sell", type="market", time_in_force="gtc")

        logging.info(f"Executed {side} trade for {ticker}, quantity: {quantity} at ${price:.2f}")
        return {"ticker": ticker, "quantity": quantity, "side": side, "price": price}
    except Exception as e:
...
def simulate_trading(crypto_tickers, stock_tickers, start_date, end_date, threshold):
    ...
    try:
        ...
        for ticker in crypto_tickers + stock_tickers:
            ...
            # Combine metrics into ISI (simple weighted average)
            isi = np.mean([sentiment, technical_scores.get("SMA", 0), technical_scores.get("EMA", 0), ml_score])

            # Decide on trade based on ISI
            if isi >= threshold:
                # Fetch latest price and determine quantity
        ...
        return {"trades": trades, "crypto_data": crypto_data, "stock_data": stock_data}
    except Exception as e:
        logging.error(f"Error during trading simulation: {e}")
        return {"trades": {}, "crypto_data": {}, "stock_data": {}}
...
dbc.Row(
    ...
    def update_dashboard(n_clicks, threshold, crypto_input, stock_input, start_date, end_date):
        ...
        try:
            ...
            buying_power_text = f"Buying Power: ${INITIAL_BALANCE - sum(trade['price'] * trade['quantity'] for trade in trades.values()):.2f}"
            pnl_text = f"PnL: ${0:.2f}"  # Placeholder

            trades_text = "\n".join([f"{trade['ticker']}: {trade['quantity']} units at ${trade['price']:.2f}" for trade in trades.values()])
