...
def create_dashboard():
    ...
    def update_dashboard(n_clicks, crypto_input, stock_input, developer_key):
        ...
        # ISM Scores vs Ticker Names
        portfolio_fig = {
            "data": [
                go.Bar(
                    x=[score["ticker"] for score in results["ISM Scores"]],  # Ticker names
                    y=[score["investment_surety"] for score in results["ISM Scores"]],  # ISM Scores
                    name="ISM Scores",
                    marker=dict(color="#FFD700"),  # Golden bars
                )
            ],
            "layout": go.Layout(
                title="ISM Scores by Ticker",
                xaxis={"title": "Ticker"},
                yaxis={"title": "ISM Score"},
                plot_bgcolor="#121212",
        ...
        strategy_fig = {
            "data": [
                go.Bar(
                    x=list(results["Strategy Results"].keys()),  # Strategy names
